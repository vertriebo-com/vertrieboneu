...
Deno.serve(async (req) => {
  ...
  const summary = {
    ...
    opportunities_ready: 'GREEN — Opportunity Entity gebaut: organization_id, company_id, title, stage (8 Stages), status, value, probability, expected_close_date, won_lost_reason, source_type, closed_at, stage_changed_at/by. Backend: listOpportunities, createOpportunity, updateOpportunityStage. UI: OpportunitySection im LeadDetail.',
    pipeline_ready: 'GREEN — Opportunity.stage (new|contacted|qualified|offer_planned|offer_sent|negotiation|won|lost). Stage-History via ContactLog. Pipeline-Wert + Forecast via listOpportunities.diagnostics. Company.lifecycle_stage-Sync bei won/lost.',
    ...
    reporting_ready: 'YELLOW → GREEN (partial) — Lead-Statistiken OK. listOpportunities.diagnostics liefert pipeline_value, weighted_forecast, won_value, open_count. Vollständige Forecast-Seite noch nicht gebaut.',
    ...
    next_mvp_block_priority_5: 'DONE: Opportunity Entity + listOpportunities + createOpportunity + updateOpportunityStage + OpportunitySection UI im LeadDetail',
    next_mvp_block_priority_6: 'ContactLog.opportunity_id + LeadOutcome.opportunity_id ergänzen (small)',
    next_mvp_block_priority_7: 'Pipeline-View /pipeline (einfache Tabelle, kein Kanban)',
