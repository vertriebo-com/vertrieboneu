...
const PAGE_MATRIX = [
  ...
  {
    page: 'Statistics.jsx',
    data_sources: ['Company', 'ContactLog', 'LeadOutcome'],
    fetch_limits: {
      Company: '500 (hard-coded)',
      ContactLog: '500 (hard-coded)',
      LeadOutcome: '500 (hard-coded)',
    },
    client_filtering: ['none — all data used for aggregation'],
    client_sorting: ['brancheData sorted by conversion rate (slice(0,10))'],
    browser_aggregation: [
      'status distribution (reduce over companies)',
      'contact type distribution (reduce over contactLogs)',
      'conversion rate (gewonnen / total)',
      'conversion per branche (map/reduce, filtered to >= 2 leads)',
      'latest outcome per lead (map over outcomes, sorted by date)',
      'outcome type counts (filter + length)',
    ],
    pagination: { server_side: false, client_side: false, verdict: 'NONE — all 500 records loaded per entity' },
    org_scoped: true,
    query_key_has_org_id: 'no React Query — raw useEffect/useState pattern',
    server_filtering_present: false,
    risk: 'red',
    risk_notes: 'Hard limit of 500 per entity. At 500+ leads: silently truncates data, aggregations become WRONG (e.g. conversion rate calculated on only 500/1000 companies). No backend aggregation. No React Query caching. Heavy browser aggregation on every render.',
    recommended_api: 'getStatisticsSummary({ org_id, period_month }) → returns pre-aggregated stats server-side',
  },
  {
    page: 'Dashboard.jsx',
  ...
  try {
    ...
    return Response.json({
      ...
      hard_values: {
        ...
        existing_backend_aggregations: ['getDashboardData', 'getUsageSummary'],
